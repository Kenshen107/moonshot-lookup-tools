...
- **Sealed Helper:** players tap the cards they opened; the − on a tile (or in the pool list, which starts open) takes one out. It counts each color's cards, creatures, removal, interaction, protection, draw, ramp and rares, suggests the best two colors, and splits 17 lands by mana symbols. Pair scores: each playable card counts 1, plus the extras in `PR_PAIR_WEIGHTS` (removal 1.5, rare/mythic 1.5, interaction 0.75, draw / ramp / protection 0.5 each); the page states them. Removal (`PR_REMOVAL_RE`), interaction (`PR_INTERACTION_RE`: counterspells, bounce, tapping down, can't block, discard, stealing) and protection (`PR_PROTECTION_RE`: granting hexproof, indestructible, protection or ward, phasing out, bouncing your own permanent) are spotted from the card text, so they're approximate. Draw (`PR_DRAW_RE`, not draw-then-discard looting) and ramp (`PR_RAMP_RE`: mana abilities, extra lands, Treasure and Heartwood tokens) are matched with reminder text and quoted abilities removed, so a Jace token's "[−3]: Draw a card" doesn't count. The interaction, protection, draw and ramp patterns were checked against Reality Fracture's cards on 2026-09-25. The pool is saved in the browser (`prSealedPool:<code>`).
- **Deck-Building Card:** a handout printed 4 per letter page (each card exactly a quarter sheet, 4.25in × 5.5in, printed with no page margin so the dashed cut lines are the page's center lines; 0.2in padding keeps text off the printer's unprintable edge). The preview can be edited by clicking; edits are saved in the browser (`prDeckCard:<code>`), and "Reset to default" undoes them. The default card is built from `PRERELEASE_SET_NOTES` (the 10 color-pair `themes`, and the short `cardName` / `card` line of each mechanic that has one). The card's "Mechanics to know" list is for mechanics a newer player could trip on, new or returning (Surveil is there because players mix it up with scry). Leave `card` off simple ones like Threshold. Without notes, it uses the cards' reminder text and each pair's two-color uncommon. `fitPrereleaseCard()` sets the text to the largest size that fits (the card's styles are in em), so the card always fills its quarter page, even after edits. The print sheets use a named `@page prsheet` with no margin, so printing other pages keeps normal margins. Always keep the two footer lines: "Use while building - put it away during games." (players can't use outside notes during games) and "Unofficial; not endorsed by Wizards of the Coast." (Wizards' Fan Content Policy). Hand the card out free; don't sell it.
