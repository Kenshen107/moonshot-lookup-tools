...
- **Sealed Helper:** players tap the cards they opened; the − on a tile (or in the pool list, which starts open) takes one out. It counts each color's cards, creatures, removal, interaction, protection and rares, suggests the best two colors (removal and rares count extra), and splits 17 lands by mana symbols. Removal (`PR_REMOVAL_RE`), interaction (`PR_INTERACTION_RE`: counterspells, bounce, tapping down, can't block, discard, stealing) and protection (`PR_PROTECTION_RE`: granting hexproof, indestructible, protection or ward, phasing out, bouncing your own permanent) are spotted from the card text, so they're approximate. The interaction and protection patterns were checked against Reality Fracture's cards on 2026-09-25. The pool is saved in the browser (`prSealedPool:<code>`).
