...
- **Market Watch tab** (`MARKET_SUBTABS`, under At the counter):
  - Price Spikes (filters for window, jump range, current price range, finish, promos, and "Hide cards from before 2003", which is on by default and uses Scryfall's set release dates). It uses ranges rather than "at least" because old, thinly traded cards swing by big dollar amounts and would fill an "at least" list.
