...
**Precon prices and reprint value:** opening a decklist prices every card at its exact printing in the deck (the foil price where the deck's copy is foil). It uses Scryfall's `/cards/collection`, 75 printings per request (`loadDeckPrintings`), which returns Scryfall's daily TCGplayer market price and its `reprint` flag. The list and picture views show each card's price. Cards worth `REPRINT_VALUE_MIN` ($5) or more are flagged: reprints get a gold 💰 Reprint badge, and cards first printed in this product get a blue 💎 New badge (as a frame in picture view). A box at the top lists both groups, most valuable first, with the deck's total value split into reprints and new cards. The commander shows its price too. A "By type / 💲 By price" toggle switches between the grouped list and one list sorted most valuable first (`productDeckSort`), in both list and picture view.
