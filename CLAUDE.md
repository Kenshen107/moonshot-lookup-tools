...
**Precon prices and reprint value:** opening a decklist prices every card at its exact printing in the deck (the foil price where the deck's copy is foil). It uses Scryfall's `/cards/collection`, 75 printings per request (`loadDeckPrintings`), which returns Scryfall's daily TCGplayer market price and its `reprint` flag. The list and picture views show each card's price. Reprints worth `REPRINT_VALUE_MIN` ($5) or more get a 💰 Reprint badge (a gold frame in picture view). A "Reprints worth $5+" box at the top lists them, most valuable first, with the deck's total value split into reprints and new cards. The commander shows its price too.
